...
export class DGService {
  ...
  direccionesGenerales() {
    return this._http.get<iDireccionesGenerales[]>(
      `${this.baseUrl}/direcciones_ctrl`
    );
  }
  direccionesGeneralesPI() {
    return this._http.get<iDireccionesGenerales[]>(
      `${this.baseUrl}/direcciones_ctrl_PI`
    );
  }
  //! get productos
  productos() {
  ...
  subcomponentes() {
    return this._http.get<Subcomponente[]>(
      `${this.baseUrl}/subcomponentes_ctrl`
    );
  }
  ...
  programasInformaName() {
    return this._http.get<ProgInformacion[]>(
      `${this.baseUrl}/prog_informacion_ctrl`
    );
  }
  ...
  actiEstaGeoName() {
    return this._http.get<iActividadEstadisticaGeografica[]>(
      `${this.baseUrl}/aeg_name_ctrl`
    );
  }
  ...
  indicadoresPS2023() {
    return this._http.get<IndicadoresPS2023[]>(
      `${this.baseUrl}/indicadores_PS_2023_ctrl`
    );
  }
  ...
  suggestionByQuery(query: string): Observable<Products[]> {
    ...
    return this._http.get<Products[]>(
      `${this.baseUrl}/fuzzy-search?q=${query}`
    );
  }

  indicadoresCollectionTable(): Observable<IndicadorTable[]> {
    return this._http.get<IndicadorTable[]>(`${this.baseUrl}/indicadores_coll`);
  }
}
